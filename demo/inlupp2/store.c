  ...
  while (true)
    {
      ...
      printf ("%d) %s %d.%d SEK  %d in stock\n", i, current_name,
              current_price / 100, current_price % 100, current_merch->theoretical_stock);
  ...
  while (true)
    {
      ...
      int current_price = current_merch->price;
      printf ("%d) %s %d.%d SEK  %d in stock\n", i, current_name,
              current_price / 100, current_price % 100, current_merch->total_stock);
      if (!ioopm_iterator_has_next (iterator))
        break;
      ioopm_iterator_next (iterator);
      if (i % 20 == 0)
      ...
      while (true)
        {
          ...
          if (!ioopm_hash_table_has_key (inventory->warehouse, (elem_t) {.ptr_v = merch->name}))
          ...
          else
            {
              ...
              printf ("\nYou bought %d pcs of %s. \nPrice: %d.%d SEK\n", merch->pcs, merch->name,
                      (merch->pcs * merch->price) / 100, (merch->pcs * merch->price) % 100);
            }
