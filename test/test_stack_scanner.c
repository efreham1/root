...
int main ()
{
  ...
  if ((CU_add_test (my_test_suite, "Test for finding pointers on the stack", test_stack_scanner) == NULL) ||
      ...
      (CU_add_test (my_test_suite, "Test for finding pointers on the stack to entrys in ht", test_find_pointer_to_entry_ht)
       == NULL) ||
      (CU_add_test (my_test_suite, "Test for finding pointers on different stack levels", test_function_variables)
       == NULL) ||
      0)
    {
