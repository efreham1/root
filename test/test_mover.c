...
int main ()
{
  ...
  if ((CU_add_test (my_test_suite, "Test for do_move with 2 structs", test_do_move) == NULL) ||
      ...
      (CU_add_test (my_test_suite, "Test for do_move with a hash table with no entries", test_do_move_ht_no_entries)
       == NULL) ||
      (CU_add_test (my_test_suite, "Test for do_move with a hash table with one entry", test_do_move_ht_one_entry)
       == NULL) ||
      (CU_add_test (my_test_suite, "Test for move with a hash table with one entry", test_move_ht_one_entry) == NULL) ||
      (CU_add_test (my_test_suite, "Test for move with a hash table with many entries", test_move_ht_many_entries)
       == NULL) ||
      (CU_add_test (my_test_suite, "Test for move with a hash table with many entries and trash", test_move_ht_many_entries_2)
       == NULL) ||
